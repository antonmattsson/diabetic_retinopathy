...
train_gen = datagen.flow_from_directory('../data/grayscale_14_train',
                                        target_size=(256, 256),
                                        batch_size=batch_size)

test_datagen = ImageDataGenerator(rescale=1./255)

test_gen = test_datagen.flow_from_directory('../data/grayscale_14_validation',
                                        target_size=(256, 256),
                                        batch_size=batch_size)

# Check if a pretrained model exists
model_path = "model_grayscale_augmented_14.h5"
...
if model_file.is_file():
    ...
    model.add(Dense(2, activation="softmax")) #change from 5 to 2, since now only to labels

    learning_rate = 0.001 # changed from 5e-4 to 0.001 (default)
...
n_epochs = 15
history_dict = None
# Save model and history dict between every 10th epoch
for i in range(n_epochs):
    ...
    model.save('model_grayscale_augmented_14.h5')
    with open('history_grayscale_augmented_14', 'wb') as file_pi:
